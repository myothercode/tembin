    ...
    public ModelAndView getUserCases(@ModelAttribute( "initSomeParmMap" )ModelMap modelMap){
        ...
        for(UsercontrollerEbayAccountExtend ebay:ebays){
            ...
            if(taskGetUserCaseses!=null&&taskGetUserCaseses.size()>0){
                ebayDates.add(taskGetUserCaseses.get(0).getLastsyctime());
            }else{
                ebayDates.add(null);
            }
        }
...
*//*            d.setSoaResponseDataformat("XML");*//*
    ...
    public void apiGetuserCasessRequest(CommonParmVO commonParmVO,HttpServletRequest request) throws Exception {
        ...
        for(Long ebay:ebays){
            if(taskGetUserCaseses!=null&&taskGetUserCaseses.size()>0){
                String ebayName=taskGetUserCaseses.get(0).getEbayname();
                ...
                if(!ebay1.getEbayName().equals(ebayName)){
                    ...
                    for(TaskGetUserCases taskGetMessages:taskGetUserCaseses){
                    ...
                    for(TaskGetUserCases taskGetMessages:taskGetUserCaseses){
                     /*   if(ebay==taskGetMessages.getEbayid()){*/
                        if(ebay1.getEbayName().equals(taskGetMessages.getEbayname())){
                            TaskGetUserCases ta=new TaskGetUserCases();
                            ta=taskGetMessages;
                            flag=flag-1;
                            ta.setTokenflag(flag);
                            ta.setLastsyctime(new Date());
                            iTaskGetUserCases.saveListTaskGetUserCases(ta);
                        }
                    }
                }
            }
